...
if (!admin.apps.length) {
  ...
  if (!projectId || !clientEmail || !privateKey) throw new Error('Missing Firebase Admin credentials in env.');
...
export default async function handler(req, res) {
  ...
  const message = {
    ...
    data: { type: 'new_order', orderId: String(order.id), url: '/owner/orders' },
    android: {
      priority: 'high',
  ...
  try {
    ...
    response.responses.forEach((r, idx) => {
      if (!r.success) {
        ...
        if (code.includes('registration-token-not-registered') || code.includes('invalid-argument')) invalid.push(tokens[idx]);
      }
    });
    if (invalid.length) await supabase.from('push_subscription_restaurants').delete().in('device_token', invalid);

    return res.status(200).json({ successCount: response.successCount, failureCount: response.failureCount, pruned: invalid.length });
  } catch (err) {
