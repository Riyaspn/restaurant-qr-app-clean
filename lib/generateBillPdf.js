...
export async function generateBillPdf(payload) {
  ...
  const servicePricesInclusive = !!invoice?.prices_include_tax
  ...
  items.forEach((it) => {
    ...
    doc.text(money(it.rate), col.rate.x, y, { width: col.rate.w, align: 'right' })
