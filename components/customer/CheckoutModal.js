...
export default function CheckoutModal({ items, restaurant, tableNumber, total, subtotal, tax, onClose, onOrderSuccess }) {
  ...
  // Store order data for "create after payment" flow
  ...
  const handleCashPayment = async () => {
    ...
    try {
      ...
      // For cash payments, create order immediately with pending payment status
  ...
  const handleOnlinePayment = () => {
    storeOrderData() // Store for after payment completion
    setShowPayment(true)
  }
        ...
        <div className="modal-content">
          ...
          <section className="info-section">
            ...
            <div className="order-info">
              ...
              <p>{items.reduce((sum, item) => sum + item.quantity, 0)} items • ₹{total.toFixed(2)}</p>
            </div>
          </section>
        ...
        <div className="modal-actions">
          ...
          >
            {loading ? 'Placing Order...' : 
             paymentMethod === 'online' ? `Pay ₹${total.toFixed(0)}` : 'Place Order'}
          </button>
        </div>
