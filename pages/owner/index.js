// pages/owner/index.js (full page for convenience)
  ...
  useEffect(() => {
    ...
    async function fetchOverview() {
      try {
        ...
        // Live orders: new, in_progress, ready
        ...
        // Today revenue and average ticket (completed)
        ...
        // Out of stock: count items marked unavailable in menu_items
        ...
        if (!cancel) {
          setStats({ liveOrders, revenueToday, avgTicket, outOfStock: outCount || 0 });
        }
      } catch (e) {
        if (!cancel) {
...
function KpiGrid({ stats }) {
  ...
  return (
    <div className="kpi-grid">
      ...
      <style jsx>{`
        .kpi-grid { display: grid; gap: 16px; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); margin-bottom: 16px; }
...
function QuickActions() {
  return (
    <>
      ...
      <style jsx>{`
        .qa { display: flex; gap: 12px; flex-wrap: wrap; margin-bottom: 16px; }
        .btn { background: #f97316; color: #fff; padding: 10px 14px; border-radius: 10px; border: 1px solid #ea580c; text-decoration: none; box-shadow: 0 6px 16px rgba(249, 115, 22, 0.15); }
      `}</style>
    </>
  );
}
  ...
  return (
    <>
      ...
      <style jsx>{`
        ...
        .pad { padding: 16px; }
        .subtle { color: #6b7280; }
        .row { display: grid; grid-template-columns: 1fr 180px 120px 100px; gap: 8px; padding: 12px 16px; border-top: 1px solid #f3f4f6; }
        .id { color: #111827; }
        .date { color: #6b7280; }
