...
export default function SettingsPage() {
      ...
      try {
        ...
        if (data) {
          setForm((prev) => ({
            ...
            restaurant_name: data.restaurant_name || data.legal_name || "",
  ...
  const onChange = (field) => (e) => {
    ...
    setForm((prev) => {
      ...
      if (field === "beneficiary_name" || field === "bank_account_holder_name") {
        return prev;
      }
  ...
  const save = async (e) => {
    ...
    try {
      ...
      if (safeTrim(form.beneficiary_name) !== safeTrim(form.legal_name))
      ...
      if (!UPI_REGEX.test(safeTrim(form.upi_id))) throw new Error("Invalid UPI format");

      const IFSC_REGEX = /^[A-Z]{4}0[A-Z0-9]{6}$/;
      const formattedIfsc = safeTrim(form.bank_ifsc).toUpperCase();
      if (!IFSC_REGEX.test(formattedIfsc)) throw new Error("Invalid IFSC code");

      const newCount = Number(form.tables_count);
      if (!isFirstTime && newCount < originalTables) throw new Error("Cannot decrease tables");
      ...
      const { error: upsertErr } = await supabase
        .from("restaurant_profiles")
        .upsert({ ...payloadProfile, route_account_id: undefined }, { onConflict: "restaurant_id" });
      ...
      if (!form.route_account_id) {
        const profile = {
          category: payloadProfile.profile_category,
          subcategory: payloadProfile.profile_subcategory,
          addresses: {
            registered: {
              street1: payloadProfile.profile_address_street1,
              street2: payloadProfile.profile_address_street2,
              city: payloadProfile.profile_address_city,
              state: payloadProfile.profile_address_state,
              postal_code: payloadProfile.profile_address_pincode,
              country: payloadProfile.profile_address_country,
            },
          },
        };
        const legal_info = {
          pan: payloadProfile.legal_pan,
        };
        if (form.gst_enabled && form.legal_gst)
          legal_info.gst = payloadProfile.legal_gst.toUpperCase();
        ...
        const res = await fetch("/api/route/create-account", {
          ...
          body: JSON.stringify({
            ...
            bank_account_name: form.bank_account_holder_name,
      ...
      // Optional email logic for tables, omitted here for brevity
  ...
  return (
    <div className="container" style={{ padding: "20px 8px 40px" }}>
      ...
      {success && (
        <Card padding={12} style={{ background: "#ecf9f1", borderColor: "#a4d4af" }}>
          <div style={{ color: "#116721" }}>{success}</div>
        </Card>
      )}
